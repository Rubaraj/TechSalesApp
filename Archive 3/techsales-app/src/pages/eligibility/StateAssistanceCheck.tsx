...
export function StateAssistanceCheckPage() {
  ...
  const handleLeadSelect = async (lead: Lead) => {
    ...
    setCheckForm({
      ...
      stateAssistanceNumber: lead.stateAssistanceNumber || lead.medicaidId || '',
      state: lead.state,
  ...
  const handleCheck = () => {
    ...
    if (!checkForm.stateAssistanceNumber || !checkForm.leadName || !checkForm.state || !checkForm.dateOfBirth) {
      alert('Please fill in all required fields');
      return;
    }

    // Simulate eligibility check using state assistance number
    setTimeout(() => {
      // Mock verification: Check if state assistance number is valid format
      const isValidFormat = /^SA-\d{8}$/.test(checkForm.stateAssistanceNumber) || 
                            /^\d{8,12}$/.test(checkForm.stateAssistanceNumber);
