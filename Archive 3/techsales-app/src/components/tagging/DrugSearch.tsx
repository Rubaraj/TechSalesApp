...
export function DrugSearch({
  ...
  const addDrug = (drug: Drug) => {
    ...
    const newTaggedDrug: TaggedDrug = {
      drugId: drug.drugId,
      drugName: drug.drugLabelName || drug.brandName || drug.genericName,
  ...
  // Get drug name for display (with fallback)
  const getDrugName = (taggedDrug: TaggedDrug): string => {
    // First try the stored drugName
    if (taggedDrug.drugName) {
      return taggedDrug.drugName;
    }
    // Try to find in current search results
    const foundDrug = drugs.find(d => d.drugId === taggedDrug.drugId);
    if (foundDrug) {
      return foundDrug.drugLabelName || foundDrug.brandName || foundDrug.genericName || 'Unknown Drug';
    }
    // Fallback to drugId if nothing found
    return `Drug ${taggedDrug.drugId}`;
  };
                ...
                <div className="p-4 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <Select
                      ...
                      value={(drug.daysSupply || 30).toString()}
