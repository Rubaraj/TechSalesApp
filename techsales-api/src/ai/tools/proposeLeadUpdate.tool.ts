...
export const proposeLeadUpdateTool = tool(
  ...
  },
  {
    ...
    description:
      "Propose updating a lead's contact details and create a pending PROPOSAL the agent " +
      'must approve before the record changes. NEVER updates directly. Updatable fields: ' +
      'phone, email, address1, address2, city, state, zipCode — nothing else (status changes ' +
      'use propose_status_change; notes use append_lead_note). Call this when the agent asks ' +
      'to change contact info (e.g. "update her phone to 555-0100", "fix John\'s email").',
    schema: inputSchema,
  },
);
