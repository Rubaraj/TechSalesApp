...
export const searchPlans = async (params: PlanSearchParams): Promise<ServiceResponse<{
  ...
  if (params.filters) {
    ...
    if (params.filters.contractYear !== undefined) {
      const contractYear = typeof params.filters.contractYear === 'string' 
        ? parseInt(params.filters.contractYear, 10) 
        : params.filters.contractYear;
      filteredPlans = filteredPlans.filter(p => p.contractYear === contractYear);
    }
    // Age filter
