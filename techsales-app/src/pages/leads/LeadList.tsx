...
export function LeadList() {
  ...
  const [filters, setFilters] = useState<LeadFilters>(() => {
    const baseFilters: LeadFilters = !isAdmin && user ? { createdBy: user.userId } : {};
    if (initialStatus) {
      baseFilters.status = initialStatus;
    }
    // Allow admin to filter by createdBy from URL
    if (isAdmin && initialCreatedBy) {
      baseFilters.createdBy = initialCreatedBy;
    }
    return baseFilters;
  });
  const [showFilters, setShowFilters] = useState(!!initialStatus || !!initialCreatedBy);
  ...
  const loadLeads = useCallback(async () => {
    setIsLoading(true);
    // For agents, always include their userId in filters
  ...
  // Load status counts (independent of current filters, but filtered by agent if not admin)
  ...
  // Clear filters (but preserve agent filter for non-admin users)
  const clearFilters = () => {
    if (!isAdmin && user) {
      setFilters({ createdBy: user.userId });
    } else {
      setFilters({});
    }
    setSearchTerm('');
